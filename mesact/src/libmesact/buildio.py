...
def build(parent):
	...
	for i in range(32):
		...
		if input_dict.get(key, False): # return False if key is not in dictionary
			...
			if key == 'Home All':
				contents.append('\n# Home All Joints\n')
				contents.append('net home-all ' + f'hm2_7i92.0.gpio.{i:02}.in{invert}\n')
				for i in range(5):
				...
				contents.append(f'net remote-estop estop-latch.0.fault-in <= hm2_7i92.0.{card}.0.{port}.input-{i:02}{invert}\n')
